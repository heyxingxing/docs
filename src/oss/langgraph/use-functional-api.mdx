...
[**Functional API**](/oss/langgraph/functional-api) 允许你在尽量少改动现有代码的前提下，为应用加入 LangGraph 的核心能力，包括[持久化](/oss/langgraph/persistence)、[记忆](/oss/langgraph/add-memory)、[人在回路](/oss/langgraph/interrupts)和[流式输出](/oss/langgraph/streaming)。

<Tip>
如果希望先理解 Functional API 的概念，请参阅 [Functional API](/oss/langgraph/functional-api)。
</Tip>

## 创建一个简单工作流

定义 `entrypoint` 时，工作流输入只能来自函数的第一个参数。如果需要传入多个输入，可以使用字典或对象。
...
<Accordion title="扩展示例：使用 LLM 撰写文章">
  这个示例展示 `@task` 与 `@entrypoint` 装饰器的基本语法。由于配置了 Checkpointer，Workflow 的执行结果会持久化到 Checkpointer 中。
...
可以并发调用多个 Task，然后等待它们的结果，从而实现并行执行。对于 I/O Bound Task（例如调用 LLM API）来说，这可以显著提高性能。
...
<Accordion title="扩展示例：并行 LLM 调用">
  这个示例展示如何使用 `@task` 并行运行多个 LLM Call。每次调用围绕一个不同 Topic 生成一段文字，最后把所有结果拼接为一段完整文本。
  ...
  这个例子利用 LangGraph 的并发模型缩短总执行时间，尤其适合 LLM Completion 等 I/O 密集型任务。
</Accordion>

## 调用 Graph

**Functional API** 与 [**Graph API**](/oss/langgraph/graph-api) 共享同一套底层 Runtime，因此可以在同一个应用中组合使用。
...
<Accordion title="扩展示例：从 Functional API 调用简单 Graph">
  :::python
  ```python
  import uuid
...
你可以在某个 **Entrypoint** 或 **Task** 内调用其他 Entrypoint。
...
**Functional API** 使用与 **Graph API** 相同的 Streaming 机制。更多细节请阅读[流式输出指南](/oss/langgraph/streaming)。

下面展示如何使用 Streaming API，在一次 Workflow Run 中流式输出 Value Chunk。
...
2. 在 Entrypoint 内获取一个 Stream Writer Instance。
3. 在计算开始前发送自定义数据。
4. 计算结束后再发送一条自定义消息。
5. 使用 `stream_events()` 处理流式输出。
6. 从 `interleave("values")` 中迭代 `(mode, chunk)` Pair。

<Warning>
**Python < 3.11 下的异步代码**
如果使用 Python < 3.11 编写异步代码，@[`get_stream_writer`] 无法正常工作。此时应直接使用 `StreamWriter` Class。详情参见 [Async with Python < 3.11](/oss/langgraph/streaming#async)。
...
4. 迭代 `stream.values` 接收 Value Snapshot。
:::
...
## 设置 Task 和 Entrypoint Timeout

使用 `@task` 或 `@entrypoint` 的 `timeout` 参数，可以限制单次异步 Attempt 最长运行时间。Timeout 可以使用秒数，也可以传 `datetime.timedelta`。
...
Timeout 只支持 Async Task 与 Async Entrypoint。如果对 Sync Function 设置 `timeout`，LangGraph 会在声明 Task 或 Entrypoint 时抛出错误。

当 Task 或 Entrypoint 超过 Timeout 后，LangGraph 会抛出 `NodeTimeoutError`；它继承自 Python 内置的 `TimeoutError`。如果 Retry Policy 会重试 `TimeoutError` 或 `NodeTimeoutError`，那么超时的 Attempt 会重新执行。Timeout 对每一次 Attempt 独立生效，因此每次 Retry 都会重新开始计时。
...
恢复执行时，无需重新运行 `slow_task`，因为它的结果已经保存在 Checkpoint 中。
...
恢复执行时，不需要重新运行 `slowTask`，因为它的结果已经保存在 Checkpoint 中。
...
Functional API 使用 @[`interrupt`] 函数和 `Command` Primitive 支持[人在回路](/oss/langgraph/interrupts)工作流。

### 基础人在回路工作流

我们创建三个 [Task](/oss/langgraph/functional-api#task)：

1. 追加 `"bar"`。
2. 暂停并等待人工输入；恢复后追加人工输入。
3. 追加 `"qux"`。
...
现在可以在一个 [Entrypoint](/oss/langgraph/functional-api#entrypoint) 中组合这些 Task：
...
[interrupt()](/oss/langgraph/interrupts#pause-using-interrupt) 在 Task 内调用，因此可以让人类审核并编辑前一个 Task 的输出。此前已经完成的 Task——这里是 `step_1`——会被持久化，因此在 @[`interrupt`] 之后恢复执行时不会重新运行。
...
注意：执行在 `step_1` 之后的 @[`interrupt`] 处暂停。Interrupt 会提供恢复 Run 所需要的信息。要继续执行，需要发送一个 [`Command`](/oss/langgraph/interrupts#resuming-interrupts)，其中包含 `human_feedback` Task 预期的数据。
...
恢复后，Run 会继续执行剩余步骤并正常结束。

### 审核 Tool Call

如果希望在 Tool 真正执行前人工审核 Tool Call，可以新增一个 `review_tool_call` 函数，并在其中调用 [`interrupt`](/oss/langgraph/interrupts#pause-using-interrupt)。调用该函数后，执行会暂停，直到收到恢复命令。

对于一条 Tool Call，我们会通过 @[`interrupt`] 请求人工审核。此时可以：

* 接受 Tool Call
* 修改 Tool Call 后继续执行
* 生成自定义 Tool Message（例如要求模型重新格式化 Tool Call）
...
现在可以更新 [Entrypoint](/oss/langgraph/functional-api#entrypoint)，审核模型生成的 Tool Call。如果 Tool Call 被接受或修改，就像之前一样执行；否则，只把人工提供的 @[`ToolMessage`] 追加到 Message History。此前已经完成的 Task——这里是第一次 Model Call——已经被持久化，因此在 @[`interrupt`] 后恢复时不会重新执行。
...
短期记忆允许在相同 **Thread ID** 的多次 **Invocation** 之间保存信息。详情参见 [Short-term Memory](/oss/langgraph/functional-api#short-term-memory)。

### 管理 Checkpoint

你可以查看和删除 Checkpointer 中保存的信息。
...
[
    ...
    StateSnapshot(
        ...
        metadata={'source': 'input', 'writes': {'__start__': {'messages': [{'role': 'user', 'content': "hi! I'm bob"}]}}, 'step': -1,
...
使用 `entrypoint.final`，可以把返回给 Caller 的值与持久化到 Checkpoint 中的值解耦。这在以下情况下很有用：

* 希望返回一个计算结果（例如 Summary 或 Status），但保存另一份内部值供下一次 Invocation 使用。
* 希望精确控制下一次 Run 中 `previous` 参数得到什么内容。
...
下面是一个使用 Functional API 与 @[`InMemorySaver`] Checkpointer 的简单 Chatbot。

这个 Bot 能够记住之前的对话，并从上次结束的位置继续。
...
[长期记忆](/oss/concepts/memory#long-term-memory)允许在不同 **Thread ID** 之间保存信息。例如，你可以在一次对话中学习某个用户的信息，然后在另一段对话中继续使用这些信息。

## Workflows

* [工作流与智能体](/oss/langgraph/workflows-agents)指南包含更多使用 Functional API 构建 Workflow 的示例。

## 与其他库集成

* [使用 Functional API 为其他 Framework 添加 LangGraph 能力](/langsmith/deploy-other-frameworks)：为原本没有内置这些能力的 Agent Framework 增加持久化、记忆与流式输出等 LangGraph 能力。